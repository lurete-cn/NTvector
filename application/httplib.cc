...
template <typename T> _ssize_t handle_EINTR(T fn) {
    _ssize_t res = 0;
  while (true) {
    res = fn();
...
_ssize_t read_socket(socket_t sock, void *ptr, size_t size, int flags) {
  return handle_EINTR([&]() {
    return recv(sock,
#ifdef _WIN32
...
_ssize_t send_socket(socket_t sock, const void *ptr, size_t size,
                           int flags) {
  return handle_EINTR([&]() {
    return send(sock,
#ifdef _WIN32
...
template <bool Read>
_ssize_t select_impl(socket_t sock, time_t sec, time_t usec) {
  struct pollfd pfd;
  pfd.fd = sock;
...
_ssize_t select_read(socket_t sock, time_t sec, time_t usec) {
  return select_impl<true>(sock, sec, usec);
}

_ssize_t select_write(socket_t sock, time_t sec, time_t usec) {
...
public:
  ...
  _ssize_t read(char *ptr, size_t size) override;
  _ssize_t write(const char *ptr, size_t size) override;
...
const char *get_header_value(const Headers &headers,
  ...
  auto it = rng.first;
  std::advance(it, static_cast<_ssize_t>(id));
...
_ssize_t write_request_line(Stream &strm, const std::string &method,
...
_ssize_t write_response_line(Stream &strm, int status) {
...
_ssize_t write_headers(Stream &strm, const Headers &headers) {
    _ssize_t write_len = 0;
...
bool parse_range_header(const std::string &s, Ranges &ranges) try {
  ...
  if (s.size() > 7 && s.compare(0, 6, "bytes=") == 0) {
    ...
    split(&s[pos], &s[pos + len], ',', [&](const char *b, const char *e) {
      ...
      const auto first =
          static_cast<_ssize_t>(lhs.empty() ? -1 : std::stoll(lhs));
      const auto last =
          static_cast<_ssize_t>(rhs.empty() ? -1 : std::stoll(rhs));
      if ((first == -1 && last == -1) ||
...
void coalesce_ranges(Ranges &ranges, size_t content_length) {
  ...
  for (auto &r : ranges) {
    ...
    if (first_pos == -1 && last_pos == -1) {
      first_pos = 0;
      last_pos = static_cast<_ssize_t>(content_length);
    }

    if (first_pos == -1) {
      first_pos = static_cast<_ssize_t>(content_length) - last_pos;
      last_pos = static_cast<_ssize_t>(content_length) - 1;
    }

    if (last_pos == -1 || last_pos >= static_cast<_ssize_t>(content_length)) {
      last_pos = static_cast<_ssize_t>(content_length) - 1;
    }
...
bool range_error(Request &req, Response &res) {
  if (!req.ranges.empty() && 200 <= res.status && res.status < 300) {
      _ssize_t content_len = static_cast<_ssize_t>(
        res.content_length_ ? res.content_length_ : res.body.size());

    std::vector<std::pair<_ssize_t, _ssize_t>> processed_ranges;
...
get_range_offset_and_length(Range r, size_t content_length) {
  ...
  assert(0 <= r.first && r.first < static_cast<_ssize_t>(content_length));
  assert(r.first <= r.second &&
         r.second < static_cast<_ssize_t>(content_length));
  (void)(content_length);
...
_ssize_t Stream::write(const char *ptr) {
  return write(ptr, strlen(ptr));
}

_ssize_t Stream::write(const std::string &s) {
...
_ssize_t SocketStream::read(char *ptr, size_t size) {
#ifdef _WIN32
  size =
      (std::min)(size, static_cast<size_t>((std::numeric_limits<int>::max)()));
#else
  size = (std::min)(size,
                    static_cast<size_t>((std::numeric_limits<_ssize_t>::max)()));
#endif
  ...
  if (read_buff_off_ < read_buff_content_size_) {
    ...
    if (size <= remaining_size) {
      ...
      read_buff_off_ += size;
      return static_cast<_ssize_t>(size);
    } else {
      memcpy(ptr, read_buff_.data() + read_buff_off_, remaining_size);
      read_buff_off_ += remaining_size;
      return static_cast<_ssize_t>(remaining_size);
    }
  }
  ...
  if (size < read_buff_size_) {
    ...
    if (n <= 0) {
      return n;
    } else if (n <= static_cast<_ssize_t>(size)) {
...
_ssize_t SocketStream::write(const char *ptr, size_t size) {
...
_ssize_t BufferStream::read(char *ptr, size_t size) {
  ...
  return static_cast<_ssize_t>(len_read);
}

_ssize_t BufferStream::write(const char *ptr, size_t size) {
...
Server &Server::set_header_writer(
    std::function<_ssize_t(Stream &, Headers &)> const &writer) {
...
_ssize_t SSLSocketStream::read(char *ptr, size_t size) {
...
_ssize_t SSLSocketStream::write(const char *ptr, size_t size) {
  if (wait_writable()) {

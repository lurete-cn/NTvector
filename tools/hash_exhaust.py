#!/usr/bin/env python3
"""穷举:全 blockStates × 序列化变体 × 哈希算法,验证 SubChunk palette 值"""
...
def nbt_block(name, states, version, mode, has_ver, intmode):
    def lp(s):
    ...
    def nint(nm, val):
        if intmode == 'zigzag': v = zig_v(val)
        elif intmode == 'plain': v = varint(val & 0xFFFFFFFF)
    ...
    inner = nstr(0x08, 'name', name) + ncomp('states', states_inner)
    if has_ver:
        inner += nint('version', version)
...
for entry in bs:
    ...
    # 精简 name 变体: 带前缀(哈希常用带前缀) 和 不带前缀
    for nm in ['minecraft:' + name, name]:
        for mode in ['littleVarint', 'little']:
            for has_ver in [True, False]:
                for intmode in ['zigzag', 'plain', 'le4']:
                    tag = nbt_block(nm, states, version, mode, has_ver, intmode)
                    for algo, h in [('neox', neox(tag)), ('fnv', fnv1a32(tag)), ('crc', zlib.crc32(tag) & 0xFFFFFFFF), ('md5', struct.unpack('<I', hashlib.md5(tag).digest()[:4])[0])]:
                        if h in tgt_set:
                            hits.setdefault(h, []).append('%s block=%s mode=%s ver=%s int=%s algo=%s' % (hex(h), nm, mode, has_ver, intmode, algo))
                            print('HIT %s : %s' % (hex(h), hits[h][-1]))

print('found %d/%d targets' % (len(hits), len(targets)))

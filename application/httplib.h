...
#ifndef HAVE_SSIZE_T      // ��� Python �Ѿ����壬������
#define HAVE_SSIZE_T
#endif
using _ssize_t = long; // �� long������ Python ����һ��
...
using Range = std::pair<_ssize_t, _ssize_t>;
...
public:
  ...
  virtual _ssize_t read(char *ptr, size_t size) = 0;
  virtual _ssize_t write(const char *ptr, size_t size) = 0;
  ...
  _ssize_t write(const char *ptr);
  _ssize_t write(const std::string &s);
};
...
public:
  ...
  Server &
  set_header_writer(std::function<_ssize_t(Stream &, Headers &)> const &writer);
...
private:
  ...
  std::function<_ssize_t(Stream &, Headers &)> header_writer_ =
...
_ssize_t send_socket(socket_t sock, const void *ptr, size_t size, int flags);
